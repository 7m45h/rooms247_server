...
def add_room(room_id, dist, addr, tele):
    room = RoomModel(room_id=room_id, dist=dist, addr=addr, tele=tele)
    db.session.add(room)
    db.session.commit()
    return [room]
...
class Room(Resource):
    ...
    def post(self):
        ...
        room = add_room(args["room_id"], args["dist"], args["addr"], args["tele"])
        room = mk_room_res(room)
        return room, 201

    def delete(self):
        args = room_delete_args.parse_args()
        room = del_room(args["room_id"])
        if room == None:
